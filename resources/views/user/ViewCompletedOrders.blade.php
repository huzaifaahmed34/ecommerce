...
function show_data(){
    ...
    $.ajax({
      ...
      url:'<?php echo url('admin/CompletedOrderShow')?>',
      async:false,
      dataType:'json',
...
var i;
 ...
 html+='<tr>'+
     ...
     '<td>'+res[i].updated_at+'</td><td>Rs '+res[i].total_price+'</td>'+
     '<td >'+res[i].payment_type+'</td>'+
     '<td><a href="#"  class="btn btn-success btn-xs" disabled>'+res[i].status+'</a></td>'+
   '<td><a href="javascript:" data='+res[i].oid+'  class="btn btn-danger btn-xs btncancel">Return</a></td>'+
     '</tr>' ;
           sno++;
