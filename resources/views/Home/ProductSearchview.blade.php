...
$subcategory_slug=''; 
$category_id='';
$product_name=request()->segment(count(request()->segments()));
$error='';
if(sizeof($res)!=0){
$category_id=$res[0]->category_id;
$subcateogry_id=$res[0]->subcategory_id;

}
else{
	$error='No Result Found For '.$product_name;
}
?>
<img src="{{asset('public/images/404/gif.gif')}}" style="display: none" width="125px;" id=gif>

	<section id="advertisement">
		<div class="container">
			<img src="{{asset('public/images/shop/advertisement.jpg')}}"  height=200px alt="" />
		</div>
	</section>
				...
				<div class="col-sm-3">
					<div class="left-sidebar">
						...
						<div class="panel-group category-products" id="accordian"><!--category-productsr-->
							@php 
							$r=DB::select("select * from categories as c where c.is_deleted=0 and c.id='$category_id'");
							$i=0;
							foreach($r as $r){
	...
	$row=DB::select("select * from sub_categories as s where s.is_deleted=0 and s.category_id=$cid");
						...
						<h2>Brand</h2>
							...
							<div class="panel panel-default">
								...
								$run=DB::select("select distinct(p.brand_name),b.brand from product as p,brand as b where b.id=p.brand_name and b.is_deleted=0 and p.product_name like '%$product_name%' ");
...
<div class="row" style="margin-bottom: 30px;">
	...
	@php 
	$coun=0;
				$run=DB::select("SELECT distinct(ps.name) as psname,ps.id as psid FROM `product_spec_values` as psv,product_spec as ps,product as p where psv.product_id=p.id and psv.product_spec_id=ps.id and p.product_name  like '%$product_name%' limit 8");
												@endphp
						@foreach($run as $run)
  ...
  <div class="panel panel-default">
    ...
    <div id="collapse{{$coun}}" class="panel-collapse collapse">
      <ul class="list-group">
      		@php	
							$coun++;	
									$r=DB::select("SELECT distinct(psv.value) as psvname FROM `product_spec_values` as psv,product_spec as ps,product as p where psv.product_id=p.id and psv.product_spec_id=ps.id  and ps.id='$run->psid' and p.product_name like '%$product_name%'");
@endphp
@foreach($r as $r)
...
<div id="image" style="display:none;width:69px;height:89px;border:1px solid black;position:absolute;top:50%;left:50%;padding:2px;"><br>Loading..</div>
				<div class="col-sm-9 padding-right">
					<div class="features_items"><!--features_items-->
						<h2 class="title text-center">Relevance Items</h2>
						...
						<div class="col-sm-4">
							<div class="product-image-wrapper">
								<div class="single-products">
									...
									<img src="data:image/png;base64,{{ chunk_split(base64_encode($r->logo)) }}" alt="" width="100%" height="200px;" />
										<?php							
				...
				?>
									...
									<?php 
											...
											<p><?php echo substr($r->product_name,0,30)?></p>
												<a href="{{url('/ProductDetail/')}}/{{$r->slug}}" class="btn btn-default add-to-cart"><i class="fa fa-shopping-cart"></i>View</a>
											<?php 
					...
					END  as stars from review as r,product as p where p.id=r.product_id and p.id='$r->id'");
...
<ul class="pagination " >
 ...
 <script src = "https://ajax.googleapis.com/ajax/libs/jquery/3.4.0/jquery.min.js"></script>
	...
	$(function(){
		...
		var slug='<?php echo $product_name ?>';
...
filter();
			...
			$.ajax({
					method:'get',
					data:{'brand':brand,'ptype':ptype,'slug':slug,'minprice':minprice,'maxprice':maxprice,'filter':filter},
					url:'<?php echo url('getFilterSearch')?>',
					dataType:'json',
					async:'false',
			cache:false,
        beforeSend: function(){
